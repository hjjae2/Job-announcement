...
def kakao_send_to_me(number, campus, company, title, href, status, r_date, d_date, KAKAO_TOKEN=""):
    ...
    while(idx < 5):
        if title[idx] != None:
            description.append(str(status[idx] + " / " + d_date[idx]))
        else:
    ...
    data = {"template_args": json.dumps(payload), "template_id": 19678}
...
def sendKakao(posts, known_posts, access_token, kakao_send_to_me):
    # Posts is a list[numbers, campuses, companys, titles, hrefs, statuss, r_dates, d_dates].
    numbers = posts[0]
    campuses = posts[1]
    companys = posts[2]
    titles = posts[3]
    ...
    known_file_path = "./data/known_internship_posts.txt"
    fw = open(known_file_path, "a", encoding='utf-8')

    print(">" * 60 + "start sending kakaotalk message ...")
    number = []; campuse = []; company = []; title = []; href = []; status = [];  r_date = []; d_date = []
    ...
    for each in range(total_number):
        ...
        if post + "\n" not in known_posts:
            ...
            if cnt % 5 == 0:
                kakao_send_to_me(number, campuse, company, title, href, status, r_date, d_date, KAKAO_TOKEN=access_token)
                number = []; campuse = []; company = []; title = []; href = []; status = []; r_date = []; d_date = []
...
if __name__ == '__main__':
    ...
    file_path = "./data/internship_posts.csv"
